...
var fare
var transNnrOohama = 0; //大浜線乗換の時1となって乗換感知
/*国鉄距離運賃計算関数*/
function getNnr(nnr) {
  if (distance == 0){
    return 0;
...
calc.addEventListener('click', function () {
  ...
  if (count == 2) {
    ...
    distance = Math.abs(disSaio[3] - distination[0]) + Math.abs(distinationOohama[0] - disOham[0])
    }
    ...
    fare = nnr;
    let msg = '<p><span style = "font-weight: bold;font-size:32px;">' + station[0] + '</span>から<span style = "font-weight: bold;font-size:32px;">' + station[1] + '</span>までの運賃は<span style = "font-weight: bold;font-size:32px;">' + fare + '円</span>です</p>';
    let result = document.getElementById('fare')
    result.innerHTML = '<div class = "subindx"><p>Succeed!</p></div>' + msg + '<a href=fare.html>リトライ</a>';
  } else if (count >= 3) {
    let result = document.getElementById('fare')
